...
def test_clearance(cf_clearance):
    import requests
    print("测试Cloudflare密钥连通性...", end='', flush=True)
    head = {
    ...
    if res:
        ...
        print(f'{bcolors.FAIL}COOKIE INVALID{bcolors.ENDC}')
    return res
...
def get_cf_clearance(force_reload=False):
    ...
    print(f"Cloudflare密钥已失效或本地存储({full_path})不存在")
    ...
    print(f"结果保存至{local_json_path}")
    return res

def main():
    print(f'{bcolors.HEADER}{bcolors.BOLD}Test: Remove local storage and test obtaining the key{bcolors.ENDC}')
    if os.path.isfile(local_json_path):
        os.remove(local_json_path)
    print(get_cf_clearance())
    print(f'{bcolors.HEADER}{bcolors.BOLD}Test: Get keys stored locally{bcolors.ENDC}')
    print(get_cf_clearance())
    print(f'{bcolors.HEADER}{bcolors.BOLD}Test: Repeat keys{bcolors.ENDC}')
    ...
    print(result_str)
    print(f'{bcolors.HEADER}{bcolors.BOLD}Test: Tampered JSON{bcolors.ENDC}')
    with open(local_json_path, 'a') as ifstream:
        ifstream.write("hahahaha")
    print(get_cf_clearance())
